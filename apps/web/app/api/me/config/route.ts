...
export async function GET() {
  ...
  return json({ ...(await toMePayload(row)), impersonating: Boolean(session.impersonated) })
}
...
export async function PUT(req: Request) {
  ...
  return json({ ...(await toMePayload(next)), impersonating: Boolean(session.impersonated) })
}
